...
feature_list = pickle.load(open('models/embiddings.pkl','rb'))
filenames = pickle.load(open('models/filenames.pkl','rb'))


def fetching_json(index):
    with open('/media/feshion_images/styles/{}.json'.format(index)) as f:
...
if __name__ =='__main__':
    ...
    streamlit.title('Fashion Clothes Products Recommender')
    ...
    if file is not None:
        if save_file(file):
            ...
            print("features of image:- ",feature)
            index = recommend(feature_list,feature)
            print("index of images:- ",index)
